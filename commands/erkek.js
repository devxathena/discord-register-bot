...
module.exports.run = async (client, message, channel, guild, args, params) => {
  ...
  const erkekrolu = process.env.ERKEK_ROLU;
  const sunucuErkekRol = guild.roles.cache.get(`${erkekrolu}`);
  ...
  if (uye) {
    ...
    if (!uye.voice.channel) {
      ...
      if (uyeKayitliRol && !uyeKayitsizRol) {
        ...
        await uye.roles.add(sunucuErkekRol);
        ...
        db.add(`erkek.${message.guild.id}.${message.author.id}`, 1);
        db.push(
